...
#include <bedrocktc/sdk/offsets/UI.hpp>
#include <EGL/egl.h>
#include <array>
#include <atomic>
#include <cstddef>
...
namespace {
using namespace bedrocktc::events;
using bedrocktc::memory::SignatureId;
...
std::array<bedrocktc::hooks::Handle, 26> handles{};
...
bool hookSignature(SignatureId id, void* detour, Function** original) {
    const auto address = bedrocktc::memory::resolve(id);
    if (!address) return false;
    bedrocktc::hooks::Handle handle = bedrocktc::hooks::install(reinterpret_cast<void*>(address), detour, reinterpret_cast<void**>(original));
...
bool itemStackHasItem(const void* itemStack) {
    ...
    auto* counter = *reinterpret_cast<void* const*>(bytes + bedrocktc::sdk::offsets::ShulkerPreview::ItemStackBaseItem);
...
AttackKind attackKindFor(void* gameMode) {
    ...
    const auto survivalAttack = bedrocktc::memory::resolve(SignatureId::SurvivalModeAttack);
...
std::string versionDetour(void* self) {
    ...
    return std::string("\xC2\xA7" "b") + std::string(bedrocktc::Name) + " v" + std::string(bedrocktc::Version) + " " + "\xC2\xA7" "fby " + "\xC2\xA7" "e" + std::string(bedrocktc::Author) + " " + "\xC2\xA7" "f- " + "\xC2\xA7" "r" + version;
}

void tickDetour(void* actor) {
    auto* player = reinterpret_cast<bedrocktc::sdk::Player*>(actor);
...
bool gameModeAttackDetour(void* gameMode, void* target, bool playPredictiveSound, const void* hitPosition) {
    ...
    AttackEvent event{kind, gameMode, reinterpret_cast<bedrocktc::sdk::Actor*>(target), reinterpret_cast<void*>(static_cast<std::uintptr_t>(playPredictiveSound)), const_cast<void*>(hitPosition)};
    bus().publish(event);
...
void* clientUpdateDetour(void* clientInstance, bool value) {
    ...
    ClientInstanceUpdateEvent event{reinterpret_cast<bedrocktc::sdk::ClientInstance*>(clientInstance)};
    bus().publish(event);
    return result;
}
...
bool hookEgl() {
    auto egl = bedrocktc::hooks::openLibrary("libEGL.so");
    if (!egl) return false;
    const auto address = bedrocktc::hooks::symbol(egl, "eglSwapBuffers");
    if (!address) {
        bedrocktc::hooks::closeLibrary(egl);
        return false;
    }
    auto handle = bedrocktc::hooks::install(reinterpret_cast<void*>(address), reinterpret_cast<void*>(swapBuffersDetour), reinterpret_cast<void**>(&swapBuffersOriginal));
    bedrocktc::hooks::closeLibrary(egl);
    if (!handle) return false;
    if (handleCount < handles.size()) handles[handleCount++] = handle;
    return true;
...
void uninstall() {
    ...
    for (std::size_t i = 0; i < handleCount; ++i) {
        if (handles[i]) bedrocktc::hooks::remove(handles[i]);
        handles[i] = nullptr;
    }
    handleCount = 0;
    installed = false;
