...
std::filesystem::path cachePath() {
    std::filesystem::path configPath = ::bedrocktc::config::ConfigManager::get().getConfigPath();
...
void refreshMapsAsync(std::string gameId, bool force, std::function<void()> callback) {
    ...
    std::thread([game]() {
        ...
        auto& entry = gCache[game];
        entry.loading = false;
        entry.error = "Network refresh is unavailable through the public Preloader SDK; cached/bundled data remains available.";
        seedBundledMapsLocked();
        auto callbacks = std::move(gCallbacks[game]);
        gCallbacks[game].clear();
