...
static void dumpSkin(void* targetPlayer) {
    ...
    void* skinRefPtr = *(void**)((uintptr_t)targetPlayer + ::bedrocktc::sdk::offsets::Player::mSkin);
    if (!skinRefPtr) return;
    
    void* skinImplSharedPtrBase = (void*)((uintptr_t)skinRefPtr + ::bedrocktc::sdk::offsets::SerializedSkinRef::mSkinImpl);
    void* threadOwner = *(void**)skinImplSharedPtrBase;
    if (!threadOwner) return;
    
    void* skinImpl = (void*)((uintptr_t)threadOwner + ::bedrocktc::sdk::offsets::ThreadOwner::mObject);
    if (!skinImpl) return;
    
    void* targetImage = (void*)((uintptr_t)skinImpl + ::bedrocktc::sdk::offsets::SerializedSkinImpl::mSkinImage);
    
    bool isPersona = *(bool*)((uintptr_t)skinImpl + ::bedrocktc::sdk::offsets::SerializedSkinImpl::mIsPersona);
    if (isPersona) {
        uintptr_t vecBegin = *(uintptr_t*)((uintptr_t)skinImpl + ::bedrocktc::sdk::offsets::SerializedSkinImpl::mSkinAnimatedImages);
        uintptr_t vecEnd   = *(uintptr_t*)((uintptr_t)skinImpl + ::bedrocktc::sdk::offsets::SerializedSkinImpl::mSkinAnimatedImages + 8);
        
        for (uintptr_t ptr = vecBegin; ptr < vecEnd; ptr += ::bedrocktc::sdk::offsets::AnimatedImageData::Size) {
            uint32_t type = *(uint32_t*)(ptr + ::bedrocktc::sdk::offsets::AnimatedImageData::mType);
            if (type == 3 || type == 2) { 
                targetImage = (void*)(ptr + ::bedrocktc::sdk::offsets::AnimatedImageData::mImage);
                if (type == 3) break;
            }
        }
    }
    
    uint32_t width = *(uint32_t*)((uintptr_t)targetImage + ::bedrocktc::sdk::offsets::SkinImage::mWidth);
    uint32_t height = *(uint32_t*)((uintptr_t)targetImage + ::bedrocktc::sdk::offsets::SkinImage::mHeight);
    
    if (width == 0 || height == 0) return;
    
    std::string* pFilteredNameTag = (std::string*)((uintptr_t)targetPlayer + ::bedrocktc::sdk::offsets::Actor::mFilteredNameTag);
    ...
    std::string configPath = ::bedrocktc::config::ConfigManager::get().getConfigPath();
    size_t lastSlash = configPath.find_last_of('/');
    std::string configDir = (lastSlash != std::string::npos) ? configPath.substr(0, lastSlash) : "/sdcard/games/BedrockTC";
    
    std::string outPath = configDir + "/" + cleanName + "_skin.png";
    
    void* pixels = *(void**)((uintptr_t)targetImage + ::bedrocktc::sdk::offsets::Image::mBytesOffset);
    if (!pixels) return;
...
static bool onAttackHook(void* mode, void* actor, void* a3, void* a4) {
    if (g_skinStealer && g_skinStealer->enabled) {
        if (g_localPlayerNative) {
            void* level_ptr = *(void**)((uintptr_t)g_localPlayerNative + ::bedrocktc::sdk::offsets::Actor::mLevel);
...
void SkinStealerModule::onInit() {
    uintptr_t getHitResultAddr = ::bedrocktc::memory::resolve(::bedrocktc::memory::SignatureId::LevelGetHitResult);
    if (getHitResultAddr) {
        getHitResultFn = reinterpret_cast<LevelGetHitResultFn>(getHitResultAddr);
    }
    
    uintptr_t getEntityAddr = ::bedrocktc::memory::resolve(::bedrocktc::memory::SignatureId::HitResultGetEntity);
    if (getEntityAddr) {
        getEntityFn = reinterpret_cast<HitResultGetEntityFn>(getEntityAddr);
    }
    
    uintptr_t isPlayerAddr = ::bedrocktc::memory::resolve(::bedrocktc::memory::SignatureId::ActorIsPlayer);
    if (isPlayerAddr) {
        isPlayerFn = reinterpret_cast<ActorIsPlayerFn>(isPlayerAddr);
    }
    
    ::bedrocktc::events::bus().subscribe<::bedrocktc::events::LocalPlayerTickEvent>([](auto& event) { onTickHook(event.player); });
    
    ::bedrocktc::events::bus().subscribe<::bedrocktc::events::AttackEvent>([](auto& event) {
