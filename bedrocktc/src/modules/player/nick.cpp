...
static void nickTick(void* _this) {
    if (g_nickMod && g_nickMod->enabled) {
        std::string* pName = (std::string*)((uintptr_t)_this + ::bedrocktc::sdk::offsets::Player::mName);
        std::string* pFilteredNameTag = (std::string*)((uintptr_t)_this + ::bedrocktc::sdk::offsets::Actor::mFilteredNameTag);
...
void NickModule::onInit() {
    if (!m_drawTextHooked) {
        uintptr_t addr = ::bedrocktc::memory::resolve(::bedrocktc::memory::SignatureId::MinecraftUIRenderContextDrawText);
        if (addr != 0) {
            ::bedrocktc::hooks::install((void*)addr, (void*)_drawText_hook, (void**)&_drawText_orig);
            m_drawTextHooked = true;
        }
    }
    
    if (!m_tickHooked) {
        ::bedrocktc::events::bus().subscribe<::bedrocktc::events::LocalPlayerTickEvent>([](auto& event) { nickTick(event.player); });
        m_tickHooked = true;
        uintptr_t setNameTagAddr = ::bedrocktc::memory::resolve(::bedrocktc::memory::SignatureId::ActorSetNameTag);
        if (setNameTagAddr != 0) {
