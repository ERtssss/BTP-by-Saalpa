...
void TimeChangerModule::onInit() {
    ...
    uintptr_t addrGetTime = ::bedrocktc::memory::resolve(::bedrocktc::memory::SignatureId::Time);
    if (addrGetTime != 0) {
        m_patchTargetGetTime = (void*)addrGetTime;
    }

    uintptr_t addrSetTime = ::bedrocktc::memory::resolve(::bedrocktc::memory::SignatureId::SetTime);
    if (addrSetTime != 0) {
...
void TimeChangerModule::applyPatch() {
    ...
    ::bedrocktc::hooks::install(m_patchTargetGetTime, (void*)_getTime_hook, (void**)&_getTime_orig);
    ::bedrocktc::hooks::install(m_patchTargetSetTime, (void*)_setTime_hook, (void**)&_setTime_orig);
    m_patched = true;
}
