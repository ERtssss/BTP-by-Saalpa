...
static void sendReQCommand() {
    ...
    if (!sendToServer) sendToServer = (decltype(sendToServer)) ::bedrocktc::memory::resolve(::bedrocktc::memory::SignatureId::LoopbackPacketSenderSendToServer);
    if (!getPacketSender) getPacketSender = (decltype(getPacketSender)) ::bedrocktc::memory::resolve(::bedrocktc::memory::SignatureId::ClientInstanceGetPacketSender);
    if (!createPacket) createPacket = (decltype(createPacket)) ::bedrocktc::memory::resolve(::bedrocktc::memory::SignatureId::MinecraftPacketsCreatePacket);
    
    if (!createPacket || !sendToServer || !getPacketSender || !::bedrocktc::core::gamehooks::clientInstance()) return;
    ...
    uintptr_t payload = (uintptr_t)pkt + ::bedrocktc::sdk::offsets::Packet::Size;
    
    
    *reinterpret_cast<std::string*>(payload + ::bedrocktc::sdk::offsets::CommandRequestPacketPayload::mCommand) = "/q " + sCurrentGameMode;
    
    
    *reinterpret_cast<uint8_t*>(payload + ::bedrocktc::sdk::offsets::CommandRequestPacketPayload::mOrigin + ::bedrocktc::sdk::offsets::CommandOriginData::mType) = 0;
    
    
    *reinterpret_cast<bool*>(payload + ::bedrocktc::sdk::offsets::CommandRequestPacketPayload::mInternalSource) = true;
    
    void* sender = getPacketSender(::bedrocktc::core::gamehooks::clientInstance());
    if (sender) {
...
static void sendConnectionCommand() {
    ...
    *reinterpret_cast<std::string*>(payload + ::bedrocktc::sdk::offsets::CommandRequestPacketPayload::mCommand) = "/connection";
    *reinterpret_cast<uint8_t*>(payload + ::bedrocktc::sdk::offsets::CommandRequestPacketPayload::mOrigin + ::bedrocktc::sdk::offsets::CommandOriginData::mType) = 0;
    *reinterpret_cast<bool*>(payload + ::bedrocktc::sdk::offsets::CommandRequestPacketPayload::mInternalSource) = true;
...
static std::string getTextPacketMessage(void* packet) {
    uintptr_t payload = (uintptr_t)packet + ::bedrocktc::sdk::offsets::Packet::Size;
    uint32_t variantIndex = *reinterpret_cast<uint32_t*>(payload + ::bedrocktc::sdk::offsets::TextPacketPayload::mVariantIndex);
    
    if (variantIndex == 1) {
        return *reinterpret_cast<std::string*>(payload + ::bedrocktc::sdk::offsets::TextPacketPayload::AuthorAndMessage::mMessage);
    } else if (variantIndex == 0 || variantIndex == 2) {
        return *reinterpret_cast<std::string*>(payload + ::bedrocktc::sdk::offsets::TextPacketPayload::MessageOnly::mMessage);
    }
    return "";
}
...
static void onHandleSetTitlePacket(void* handler, void* source, void* packet) {
    ...
    if (AutoReQ::instance && AutoReQ::instance->enabled) {
        uintptr_t payload = (uintptr_t)packet + ::bedrocktc::sdk::offsets::Packet::Size;
        int packetType = *reinterpret_cast<int*>(payload + ::bedrocktc::sdk::offsets::SetTitlePacketPayload::mType);
        
        if (packetType == 0 || packetType == 1) { 
            std::string text = *reinterpret_cast<std::string*>(payload + ::bedrocktc::sdk::offsets::SetTitlePacketPayload::mTitleText);
            if (checkTitle(text)) {
                sendReQCommand();
            }
        }
...
static void onChangeDimension(void* player, void* packet) {
    if (AutoReQ::instance && AutoReQ::instance->enabled) {
        ...
        int dimId = *reinterpret_cast<int*>(payload + ::bedrocktc::sdk::offsets::ChangeDimensionPacketPayload::mDimensionId);
...
void AutoReQ::onInit() {
    uintptr_t setTitleAddr = ::bedrocktc::memory::resolve(::bedrocktc::memory::SignatureId::ClientNetworkHandlerHandleSetTitle);
    if (setTitleAddr) {
        ::bedrocktc::hooks::install((void*)setTitleAddr, (void*)onHandleSetTitlePacket, (void**)&onHandleSetTitlePacket_orig);
    }
    
    uintptr_t setTxtAddr = ::bedrocktc::memory::resolve(::bedrocktc::memory::SignatureId::ClientNetworkHandlerHandleText);
    if (setTxtAddr) {
        ::bedrocktc::hooks::install((void*)setTxtAddr, (void*)onHandleTextPacket, (void**)&onHandleTextPacket_orig);
    }
    
    uintptr_t changeDimAddr = ::bedrocktc::memory::resolve(::bedrocktc::memory::SignatureId::LocalPlayerChangeDimension);
    if (changeDimAddr) {
        ::bedrocktc::hooks::install((void*)changeDimAddr, (void*)onChangeDimension, (void**)&onChangeDimension_orig);
    }
}
