...
using Color = ::bedrocktc::sdk::Color;
...
void applyScreenGlintColor(void* screenContext) {
    ...
    using namespace ::bedrocktc::sdk::offsets;

    const auto constants = ::bedrocktc::sdk::field<std::uintptr_t>(
        screenContext,
        ScreenContext::mActorShaderConstants
    );
    if (!constants) return;

    const auto glintConstant = ::bedrocktc::sdk::field<std::uintptr_t>(
        reinterpret_cast<void*>(constants),
        ActorShaderConstants::mGlintColor
    );
    if (!glintConstant) return;

    const auto data = ::bedrocktc::sdk::field<std::uintptr_t>(
    ...
    *reinterpret_cast<Color*>(data) = makeGlintColor();
    ::bedrocktc::sdk::field<std::uint8_t>(
        reinterpret_cast<void*>(glintConstant),
...
void GlintColorModule::onInit() {
    const auto entityAddress = ::bedrocktc::memory::resolve(
        ::bedrocktc::memory::SignatureId::ActorShaderManagerSetEntityConstants
    );
    const auto actorGlintAddress = ::bedrocktc::memory::resolve(
        ::bedrocktc::memory::SignatureId::ActorShaderManagerSetupShaderParametersActorGlint
    );
    const auto foilAddress = ::bedrocktc::memory::resolve(
        ::bedrocktc::memory::SignatureId::ActorShaderManagerSetupFoilShaderParameters
    );
    const auto uiAddress = ::bedrocktc::memory::resolve(
        ::bedrocktc::memory::SignatureId::ActorShaderManagerSetupShaderParametersGlint
    );
...
void GlintColorModule::installHooks() {
    if (!m_entityHooked && m_entityTarget) {
        const auto hook = ::bedrocktc::hooks::install(
            m_entityTarget,
