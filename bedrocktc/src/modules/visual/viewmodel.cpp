                             ...
                             int useMatrixAsIs, int renderingMainHand)
{
    if (g_viewModelMod && g_viewModelMod->enabled) {
        ...
        if (!skip) {
            ...
            uintptr_t ptr1 = *(uintptr_t*)(rcBase + ::bedrocktc::sdk::offsets::RenderContext::mMatrixStackWrapper);
            if (ptr1 != 0) {
                uintptr_t matStack = *(uintptr_t*)(ptr1 + ::bedrocktc::sdk::offsets::MatrixStackWrapper::mMatrixStack);
                if (matStack != 0) {
                    uintptr_t* blocks = *(uintptr_t**)(matStack + ::bedrocktc::sdk::offsets::MatrixStack::mBlocks);
                    size_t start = *(size_t*)(matStack + ::bedrocktc::sdk::offsets::MatrixStack::mStart);
                    size_t size  = *(size_t*)(matStack + ::bedrocktc::sdk::offsets::MatrixStack::mSize);
...
void ViewModelModule::onInit() {
    if (!m_renderItemHooked) {
        uintptr_t addr = ::bedrocktc::memory::resolve(::bedrocktc::memory::SignatureId::RenderItem);
        if (addr != 0) {
            ::bedrocktc::hooks::install((void*)addr, (void*)_renderItem_hook, (void**)&_renderItem_orig);
    ...
    if (!m_fovHooked) {
        ...
        if (addr != 0) {
            ::bedrocktc::hooks::install((void*)addr, (void*)_getFov_hook, (void**)&_getFov_orig);
            m_fovHooked = true;
        }
    }

    if (!m_perspectiveHooked) {
        uintptr_t addr = ::bedrocktc::memory::resolve(::bedrocktc::memory::SignatureId::GetPerspective);
        if (addr != 0) {
            ::bedrocktc::hooks::install((void*)addr, (void*)_getPerspective_hook, (void**)&_getPerspective_orig);
