...
static MaterialPtr getMaterial(const char* name) {
    ...
    if (!vtable || !vtable[::bedrocktc::sdk::offsets::VTable::RenderMaterialGroup_getMaterial]) return {};
    using getMat_t = MaterialPtr(*)(void*, const HashedString*);
    return reinterpret_cast<getMat_t>(vtable[::bedrocktc::sdk::offsets::VTable::RenderMaterialGroup_getMaterial])((void*)s_renderMaterialGroup, &hs);
}
...
static void drawDigit(void* tessellator, int digit, ::bedrocktc::sdk::Vec3 center, ::bedrocktc::sdk::Vec3 right, ::bedrocktc::sdk::Vec3 up, float scale) {
    auto emitLine = [&](float x1, float y1, float x2, float y2) {
        ::bedrocktc::sdk::Vec3 p1 = {center.x + (right.x * x1 * scale) + (up.x * y1 * scale),
                   center.y + (right.y * x1 * scale) + (up.y * y1 * scale),
                   center.z + (right.z * x1 * scale) + (up.z * y1 * scale)};
        ::bedrocktc::sdk::Vec3 p2 = {center.x + (right.x * x2 * scale) + (up.x * y2 * scale),
...
static void drawNumber(void* tessellator, int number, ::bedrocktc::sdk::Vec3 center, ::bedrocktc::sdk::Vec3 right, ::bedrocktc::sdk::Vec3 up, float scale) {
    ...
    if (number < 10) {
        ...
        int tens = number / 10;
        int ones = number % 10;
        ::bedrocktc::sdk::Vec3 offsetLeft = {center.x - right.x * 0.45f * scale, center.y - right.y * 0.45f * scale, center.z - right.z * 0.45f * scale};
        ::bedrocktc::sdk::Vec3 offsetRight = {center.x + right.x * 0.45f * scale, center.y + right.y * 0.45f * scale, center.z + right.z * 0.45f * scale};
    ...
    if (!g_localPlayer) {
        ...
        if (tessellatorPtr && tessellatorPtr >= 0x1000) {
            ...
            if (lrpPtr && lrpPtr >= 0x1000) {
                ...
                void* matOutline = s_matSelection ? (void*)&s_matSelection : (void*)(lrpPtr + ::bedrocktc::sdk::offsets::LevelRendererPlayer::mSelectionOverlayMaterial);
                ...
                if (colorHolderPtr && colorHolderPtr >= 0x1000) {
                    ...
                    uintptr_t dimension = *(uintptr_t*)((uintptr_t)g_localPlayer + ::bedrocktc::sdk::offsets::Actor::mDimension);
                    if (dimension) {
                        uintptr_t blockSource = *(uintptr_t*)(dimension + ::bedrocktc::sdk::offsets::Dimension::mBlockSource);
                        if (blockSource) {
                            ...
                            for (int x = -rx; x <= rx; ++x) {
                                for (int y = -ry; y <= ry; ++y) {
                                    for (int z = -rz; z <= rz; ++z) {
                                        ...
                                        struct FaceData {
                                            BlockPos offset;
                                            ::bedrocktc::sdk::Vec3 centerOffset;
                                            ::bedrocktc::sdk::Vec3 right;
                                            ::bedrocktc::sdk::Vec3 up;
                                            bool shouldCheck;
                                        };
                                        ...
                                        for (int f = 0; f < 6; ++f) {
                                            ...
                                            if (!s_isSolidBlockingBlock(region, np)) {
                                                ...
                                                ::bedrocktc::sdk::Vec3 centerOrigin = {bp.x + faces[f].centerOffset.x, bp.y + faces[f].centerOffset.y, bp.z + faces[f].centerOffset.z};
                                                centerOrigin.x -= camX;
                                                centerOrigin.y -= camY;
                                                centerOrigin.z -= camZ;
...
void LightOverlayModule::onInit() {
    ...
    uintptr_t gb = ::bedrocktc::memory::resolve(::bedrocktc::memory::SignatureId::BlockSourceGetBlock);
    if (gb) s_getBlock = (BlockSource_getBlock_t)gb;

    uintptr_t gbr = ::bedrocktc::memory::resolve(::bedrocktc::memory::SignatureId::BlockSourceGetBrightness);
    if (gbr) s_getBrightness = (BlockSource_getBrightness_t)gbr;

    uintptr_t isb = ::bedrocktc::memory::resolve(::bedrocktc::memory::SignatureId::BlockSourceIsSolidBlockingBlock);
    if (isb) s_isSolidBlockingBlock = (BlockSource_isSolidBlockingBlock_t)isb;

    ::bedrocktc::events::bus().subscribe<::bedrocktc::events::LocalPlayerTickEvent>([](auto& event) { s_lightOverlayTickCallback(event.player); });
}
