...
void* getEntityDataWrapper(void* actor) {
    ...
    return reinterpret_cast<void*>(
        reinterpret_cast<std::uintptr_t>(actor) + ::bedrocktc::sdk::offsets::Actor::mEntityData
    );
}
...
void* getEntityContext(void* actor) {
    if (!actor) return nullptr;
    return reinterpret_cast<void*>(
        reinterpret_cast<std::uintptr_t>(actor) + ::bedrocktc::sdk::offsets::Actor::mEntityContext
    );
}
...
void* findSCharDataItemVtable(void* component) {
    ...
    for (std::size_t i = 0; i < size; ++i) {
        ...
        const auto type = *reinterpret_cast<const std::uint8_t*>(
            address + ::bedrocktc::sdk::offsets::DataItem::mType
        );
...
bool readAlwaysShowItem(void* actor, std::int8_t& value) {
    ...
    constexpr std::size_t id = ::bedrocktc::sdk::offsets::ActorDataIds::NametagAlwaysShow;
    ...
    const auto itemId = *reinterpret_cast<const std::uint16_t*>(
        address + ::bedrocktc::sdk::offsets::DataItem::mId
    );
    if (type != 0 || itemId != id) return false;

    value = *reinterpret_cast<const std::int8_t*>(
        address + ::bedrocktc::sdk::offsets::DataItem::mValue
    );
    return true;
}
...
bool writeAlwaysShowItem(void* actor, std::int8_t value) {
    ...
    constexpr std::uint16_t id = static_cast<std::uint16_t>(
        ::bedrocktc::sdk::offsets::ActorDataIds::NametagAlwaysShow
    );
    ...
    if (!item) {
        ...
        *reinterpret_cast<std::uint8_t*>(
            reinterpret_cast<std::uintptr_t>(item) + ::bedrocktc::sdk::offsets::DataItem::mType
        ) = 0;
        *reinterpret_cast<std::uint16_t*>(
            reinterpret_cast<std::uintptr_t>(item) + ::bedrocktc::sdk::offsets::DataItem::mId
        ) = id;
        begin[id] = item;
    }
...
int readFuseTicks(void* actor) {
    ...
    constexpr std::size_t fuseId = ::bedrocktc::sdk::offsets::ActorDataIds::FuseTime;
    ...
    const auto type = *reinterpret_cast<const std::uint8_t*>(
        itemAddress + ::bedrocktc::sdk::offsets::DataItem::mType
    );
    const auto id = *reinterpret_cast<const std::uint16_t*>(
        itemAddress + ::bedrocktc::sdk::offsets::DataItem::mId
    );
    if (type != ::bedrocktc::sdk::offsets::DataItem::IntType || id != fuseId) return -1;

    return *reinterpret_cast<const int*>(
        itemAddress + ::bedrocktc::sdk::offsets::DataItem::mValue
    );
}
...
void TntTimerModule::onInit() {
    g_getNameTag = reinterpret_cast<ActorGetNameTagFn>(
        ::bedrocktc::memory::resolve(::bedrocktc::memory::SignatureId::ActorGetNameTag)
    );
    g_setNameTag = reinterpret_cast<ActorSetNameTagFn>(
        ::bedrocktc::memory::resolve(::bedrocktc::memory::SignatureId::ActorSetNameTag)
    );
    g_ensureIndex = reinterpret_cast<SynchedActorDataEnsureIndexFn>(
        ::bedrocktc::memory::resolve(::bedrocktc::memory::SignatureId::SynchedActorDataEnsureIndex)
    );
    g_updateAlwaysShowNameTag = reinterpret_cast<ActorSynchedDataUpdateAlwaysShowNameTagFn>(
        ::bedrocktc::memory::resolve(
            ::bedrocktc::memory::SignatureId::ActorSynchedDataUpdateAlwaysShowNameTag
        )
    );

    const auto normalTick = ::bedrocktc::memory::resolve(
        ::bedrocktc::memory::SignatureId::PrimedTntNormalTick
    );
