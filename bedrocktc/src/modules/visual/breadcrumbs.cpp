...
static void s_breadcrumbsTickCallback(void* _this) {
    ...
    if (g_breadcrumbsMod->tickCounter >= g_breadcrumbsMod->tickInterval) {
        ...
        if (svc != 0) {
            ::bedrocktc::sdk::Vec3 pos = *(::bedrocktc::sdk::Vec3*)svc;
            ...
            if (!g_breadcrumbsMod->points.empty()) {
                ::bedrocktc::sdk::Vec3 lastPos = g_breadcrumbsMod->points.back();
...
static void _renderLevel_hook(void* _this, void* screenContext, void* a3) {
    ...
    std::vector<::bedrocktc::sdk::Vec3> points;
    {
        std::lock_guard<std::mutex> lock(s_pointsMutex);
    ...
    if (!points.empty()) {
        ...
        auto emitLine = [&](::bedrocktc::sdk::Vec3 p1, ::bedrocktc::sdk::Vec3 p2) {
        ...
        for (size_t i = 0; i < points.size(); ++i) {
            ...
            ::bedrocktc::sdk::Vec3 c1 = {bx, by, bz};
            ::bedrocktc::sdk::Vec3 c2 = {bx + 1.0f, by, bz};
            ::bedrocktc::sdk::Vec3 c3 = {bx + 1.0f, by, bz + 1.0f};
            ::bedrocktc::sdk::Vec3 c4 = {bx, by, bz + 1.0f};
            ...
            if (i > 0) {
                ::bedrocktc::sdk::Vec3 prevP = points[i - 1];
                float pbx = std::floor(prevP.x);
                float pby = prevP.y + 0.05f;
                float pbz = std::floor(prevP.z);
                
                ::bedrocktc::sdk::Vec3 centerPrev = {pbx + 0.5f, pby, pbz + 0.5f};
                ::bedrocktc::sdk::Vec3 centerCurr = {bx + 0.5f, by, bz + 0.5f};
        ...
        if (points.size() > 1) {
            ...
            for (size_t i = 1; i < points.size(); ++i) {
                ...
                ::bedrocktc::sdk::Vec3 dir = {centerCurr.x - centerPrev.x, centerCurr.y - centerPrev.y, centerCurr.z - centerPrev.z};
                float len = std::sqrt(dir.x*dir.x + dir.y*dir.y + dir.z*dir.z);
                if (len > 0.001f) {
                    dir.x /= len; dir.y /= len; dir.z /= len;
                    ::bedrocktc::sdk::Vec3 right = {dir.z, 0.0f, -dir.x}; 
                    
                    float arrowSize = 0.45f;
                    ::bedrocktc::sdk::Vec3 arrowTip = {centerCurr.x - dir.x * 0.05f, centerCurr.y - dir.y * 0.05f, centerCurr.z - dir.z * 0.05f}; 
                    ::bedrocktc::sdk::Vec3 w1 = {arrowTip.x - dir.x * arrowSize + right.x * (arrowSize * 0.8f), 
                               arrowTip.y - dir.y * arrowSize + right.y * (arrowSize * 0.8f), 
                               arrowTip.z - dir.z * arrowSize + right.z * (arrowSize * 0.8f)};
                    ::bedrocktc::sdk::Vec3 w2 = {arrowTip.x - dir.x * arrowSize - right.x * (arrowSize * 0.8f), 
                               arrowTip.y - dir.y * arrowSize - right.y * (arrowSize * 0.8f), 
                               arrowTip.z - dir.z * arrowSize - right.z * (arrowSize * 0.8f)};
...
void BreadcrumbsModule::onInit() {
    ...
    ::bedrocktc::events::bus().subscribe<::bedrocktc::events::LocalPlayerTickEvent>([](auto& event) { s_breadcrumbsTickCallback(event.player); });
}
