...
struct FaceHook {
    ::bedrocktc::hooks::Handle handle = nullptr;
    FaceFn original = nullptr;
};
FaceHook g_faceHooks[6] = {};

::bedrocktc::hooks::Handle g_paneTessHook = nullptr;
::bedrocktc::hooks::Handle g_bgGetTextureHook = nullptr;
::bedrocktc::hooks::Handle g_tessVertexHook = nullptr;
::bedrocktc::hooks::Handle g_boUpdateRenderFaceHook = nullptr;
...
GlassInfo classifyGlass(const void* block) {
    if (!block) return {};
    const auto* blockObj = static_cast<const ::bedrocktc::sdk::Block*>(block);
...
private:
    alignas(::bedrocktc::sdk::offsets::TextureUVCoordinateSet::Alignment)
        std::array<std::byte, ::bedrocktc::sdk::offsets::TextureUVCoordinateSet::Size> m_storage{};
    bool m_constructed = false;
};

void cropTexture(void* texture, const CropEdges& edges) {
    float& u0 = field<float>(texture, ::bedrocktc::sdk::offsets::TextureUVCoordinateSet::mU0);
    float& v0 = field<float>(texture, ::bedrocktc::sdk::offsets::TextureUVCoordinateSet::mV0);
    float& u1 = field<float>(texture, ::bedrocktc::sdk::offsets::TextureUVCoordinateSet::mU1);
    float& v1 = field<float>(texture, ::bedrocktc::sdk::offsets::TextureUVCoordinateSet::mV1);
    ...
    field<uint8_t>(
        texture,
        ::bedrocktc::sdk::offsets::TextureUVCoordinateSet::mIsotropicFaceData
            + ::bedrocktc::sdk::offsets::IsotropicFaceData::mTextureIsotropic
    ) = 0;
}
...
public:
    FaceStateGuard(void* tessellator, GlassFace face)
        : m_flip(&field<uint8_t>(
              tessellator,
              ::bedrocktc::sdk::offsets::BlockTessellator::mFlipFace
                  + static_cast<size_t>(face) * ::bedrocktc::sdk::offsets::FlipFace::ElementSize
          )),
          m_xFlip(&field<uint8_t>(tessellator, ::bedrocktc::sdk::offsets::BlockTessellator::mXFlipTexture)),
          m_oldFlip(*m_flip), m_oldXFlip(*m_xFlip) {
        *m_flip = ::bedrocktc::sdk::offsets::FlipFace::DontRotate;
        *m_xFlip = 0;
    }
...
void renderFace(
    ...
    do {
        ...
        const void* internalTexture = reinterpret_cast<const void*>(
            reinterpret_cast<uintptr_t>(tessellator) + ::bedrocktc::sdk::offsets::BlockTessellator::mInternalTexture
        );
        if (field<uint8_t>(tessellator, ::bedrocktc::sdk::offsets::BlockTessellator::mUseInternalTexture) != 0
            && inputTexture == internalTexture) break;
        void* region = field<void*>(tessellator, ::bedrocktc::sdk::offsets::BlockTessellator::mRegion);
        if (!region) break;
...
constexpr std::array<::bedrocktc::memory::SignatureId, 6> kFaceSigIds = {
    ::bedrocktc::memory::SignatureId::BlockTessellatorTessellateFaceDown,
    ::bedrocktc::memory::SignatureId::BlockTessellatorTessellateFaceUp,
    ::bedrocktc::memory::SignatureId::BlockTessellatorTessellateFaceNorth,
    ::bedrocktc::memory::SignatureId::BlockTessellatorTessellateFaceSouth,
    ::bedrocktc::memory::SignatureId::BlockTessellatorTessellateFaceWest,
    ::bedrocktc::memory::SignatureId::BlockTessellatorTessellateFaceEast,
};
...
bool paneTessDetour(void* tess, void* meshtess, const void* block, const BlockPosRaw* pos, int32_t layer) {
    ...
    if (shouldIntercept) {
        void* region = field<void*>(tess, ::bedrocktc::sdk::offsets::BlockTessellator::mRegion);
        const GlassInfo current = classifyGlass(block);
        g_paneRc = {
            true, meshtess, *pos,
...
const void* bgGetTextureDetour(void* bgThis, const BlockPosRaw* pos, size_t faceIdx, uint32_t variant) {
    ...
    if (faceIdx == 0 && !g_paneRc.haveSource) {
        g_paneRc.source = {
            field<float>(result, ::bedrocktc::sdk::offsets::TextureUVCoordinateSet::mU0),
            field<float>(result, ::bedrocktc::sdk::offsets::TextureUVCoordinateSet::mV0),
            field<float>(result, ::bedrocktc::sdk::offsets::TextureUVCoordinateSet::mU1),
            field<float>(result, ::bedrocktc::sdk::offsets::TextureUVCoordinateSet::mV1),
        };
...
void tessVertexDetour(void* tess, float x, [[maybe_unused]] float y, float z) {
    ...
    if (g_paneRc.active && g_paneRc.haveSource && g_paneRc.tess == tess) {
        float& u = field<float>(tess, ::bedrocktc::sdk::offsets::Tessellator::mTextureU);
        float& v = field<float>(tess, ::bedrocktc::sdk::offsets::Tessellator::mTextureV);
...
::bedrocktc::hooks::Handle installHook(::bedrocktc::memory::SignatureId id, void* detour, Fn* original) {
    const uintptr_t address = ::bedrocktc::memory::resolve(id);
    if (!address) return nullptr;
    return ::bedrocktc::hooks::install(reinterpret_cast<void*>(address), detour, reinterpret_cast<void**>(original));
}

bool rebuildRenderChunks(void* clientInstance) {
    if (!clientInstance || !g_setAllDirty) return false;

    void* levelRenderer = field<void*>(clientInstance, ::bedrocktc::sdk::offsets::ClientInstance::mLevelRenderer);
    if (!levelRenderer) return false;

    void* node = field<void*>(
        levelRenderer,
        ::bedrocktc::sdk::offsets::LevelRenderer::mRenderChunkCoordinators
            + ::bedrocktc::sdk::offsets::HashTable::mFirstNode
    );

    bool rebuilt = false;
    size_t visited = 0;
    while (node && visited++ < ::bedrocktc::sdk::offsets::RenderChunkCoordinator::MaxNodes) {
        void* next = field<void*>(node, ::bedrocktc::sdk::offsets::HashNode::mNext);
        void* coordinator = field<void*>(node, ::bedrocktc::sdk::offsets::HashNode::mValuePointer);
        if (coordinator) {
...
void ConnectedGlassModule::installHooks() {
    ...
    g_getBlock = reinterpret_cast<BlockSourceGetBlockFn>(::bedrocktc::memory::resolve(::bedrocktc::memory::SignatureId::BlockSourceGetBlockForTessellation));
    g_isSolidBlockingBlock = reinterpret_cast<BlockSourceIsSolidBlockingBlockFn>(::bedrocktc::memory::resolve(::bedrocktc::memory::SignatureId::BlockSourceIsSolidBlockingBlock));
    g_textureCopyCtor = reinterpret_cast<TextureUVCopyCtorFn>(::bedrocktc::memory::resolve(::bedrocktc::memory::SignatureId::TextureUVCoordinateSetCopyCtor));
    g_textureDtor = reinterpret_cast<TextureUVDtorFn>(::bedrocktc::memory::resolve(::bedrocktc::memory::SignatureId::TextureUVCoordinateSetDtor));
    g_setAllDirty = reinterpret_cast<SetAllDirtyFn>(::bedrocktc::memory::resolve(::bedrocktc::memory::SignatureId::RenderChunkCoordinatorSetAllDirty));
    ...
    if (!g_boUpdateRenderFaceHook) {
        g_boUpdateRenderFaceHook = installHook(
            ::bedrocktc::memory::SignatureId::BlockOccluderUpdateRenderFace,
...
void ConnectedGlassModule::onInit() {
    applySettings();
    installHooks();
    ::bedrocktc::events::bus().subscribe<::bedrocktc::events::ClientInstanceUpdateEvent>([](auto& event) {
