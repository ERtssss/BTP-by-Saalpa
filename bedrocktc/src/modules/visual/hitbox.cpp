...
static ::bedrocktc::sdk::Vec3 g_playerPos = {0.f, 0.f, 0.f};
static void* g_localPlayerPtr = nullptr;

struct AABB {
    ::bedrocktc::sdk::Vec3 min;
    ::bedrocktc::sdk::Vec3 max;
};

static void s_hitboxTickCallback(void* _this) {
    if (!g_hitboxMod || !g_hitboxMod->enabled) return;
    g_localPlayerPtr = _this;
    uintptr_t svc = *(uintptr_t*)((uintptr_t)_this + ::bedrocktc::sdk::offsets::Actor::mStateVectorComponent);
    if (svc != 0) {
        g_playerPos = *(::bedrocktc::sdk::Vec3*)svc;
    }
}
...
static AABB getActorAABB(void* actor) {
    ...
    uintptr_t builtInPtr = *(uintptr_t*)(actorAddr + ::bedrocktc::sdk::offsets::Actor::mStateVectorComponent);
    if (builtInPtr) {
        uintptr_t aabbComponentPtr = *(uintptr_t*)(actorAddr + ::bedrocktc::sdk::offsets::Actor::mStateVectorComponent + ::bedrocktc::sdk::offsets::BuiltInActorComponents::mAABBShapeComponent);
        if (aabbComponentPtr) {
            aabb = *(AABB*)(aabbComponentPtr + ::bedrocktc::sdk::offsets::AABBShapeComponent::mAABB);
        }
    }
...
static ::bedrocktc::sdk::Vec2 getActorRotation(void* actor) {
    ::bedrocktc::sdk::Vec2 rot = {0.f, 0.f};
    uintptr_t actorAddr = (uintptr_t)actor;
    uintptr_t rotComp = *(uintptr_t*)(actorAddr + ::bedrocktc::sdk::offsets::Actor::mActorRotationComponent);
    if (rotComp) {
        rot = *(::bedrocktc::sdk::Vec2*)rotComp;
    }
    return rot;
}
...
static bool hasCategory(void* actor, uint32_t categoryBit) {
    uintptr_t actorAddr = (uintptr_t)actor;
    uint32_t categories = *(uint32_t*)(actorAddr + ::bedrocktc::sdk::offsets::Actor::mCategories);
...
static void _renderLevel_hook(void* _this, void* screenContext, void* a3) {
    ...
    uintptr_t tessellatorPtr = *(uintptr_t*)((uintptr_t)screenContext + ::bedrocktc::sdk::offsets::ScreenContext::mTessellator);
    if (!tessellatorPtr || tessellatorPtr < 0x1000) return;
    void* tessellator = (void*)tessellatorPtr;

    uintptr_t lrpPtr = *(uintptr_t*)((uintptr_t)_this + ::bedrocktc::sdk::offsets::LevelRenderer::mLevelRendererPlayer);
    if (!lrpPtr || lrpPtr < 0x1000) return;

    float camX = *(float*)(lrpPtr + ::bedrocktc::sdk::offsets::LevelRendererPlayer::mCamPos);
    float camY = *(float*)(lrpPtr + ::bedrocktc::sdk::offsets::LevelRendererPlayer::mCamPos + 4);
    float camZ = *(float*)(lrpPtr + ::bedrocktc::sdk::offsets::LevelRendererPlayer::mCamPos + 8);

    ensureMaterials();

    void* matInner = s_matSelection ? (void*)&s_matSelection
                                    : (void*)(lrpPtr + ::bedrocktc::sdk::offsets::LevelRendererPlayer::mSelectionOverlayMaterial);

    uintptr_t colorHolderPtr = *(uintptr_t*)((uintptr_t)screenContext + ::bedrocktc::sdk::offsets::ScreenContext::mColorHolder);
    ...
    auto drawLines = [&](const std::vector<std::pair<::bedrocktc::sdk::Vec3, ::bedrocktc::sdk::Vec3>>& lines, uint32_t color) {
        ...
        for (const auto& line : lines) {
            ::bedrocktc::sdk::Vec3 p1 = line.first;
            ::bedrocktc::sdk::Vec3 p2 = line.second;
    ...
    auto drawBox = [&](const AABB& aabb, uint32_t color) {
        std::vector<std::pair<::bedrocktc::sdk::Vec3, ::bedrocktc::sdk::Vec3>> lines;
        ::bedrocktc::sdk::Vec3 mn = aabb.min;
        ::bedrocktc::sdk::Vec3 mx = aabb.max;

        lines.push_back({::bedrocktc::sdk::Vec3{mn.x, mn.y, mn.z}, ::bedrocktc::sdk::Vec3{mx.x, mn.y, mn.z}});
        lines.push_back({::bedrocktc::sdk::Vec3{mx.x, mn.y, mn.z}, ::bedrocktc::sdk::Vec3{mx.x, mn.y, mx.z}});
        lines.push_back({::bedrocktc::sdk::Vec3{mx.x, mn.y, mx.z}, ::bedrocktc::sdk::Vec3{mn.x, mn.y, mx.z}});
        lines.push_back({::bedrocktc::sdk::Vec3{mn.x, mn.y, mx.z}, ::bedrocktc::sdk::Vec3{mn.x, mn.y, mn.z}});

        lines.push_back({::bedrocktc::sdk::Vec3{mn.x, mx.y, mn.z}, ::bedrocktc::sdk::Vec3{mx.x, mx.y, mn.z}});
        lines.push_back({::bedrocktc::sdk::Vec3{mx.x, mx.y, mn.z}, ::bedrocktc::sdk::Vec3{mx.x, mx.y, mx.z}});
        lines.push_back({::bedrocktc::sdk::Vec3{mx.x, mx.y, mx.z}, ::bedrocktc::sdk::Vec3{mn.x, mx.y, mx.z}});
        lines.push_back({::bedrocktc::sdk::Vec3{mn.x, mx.y, mx.z}, ::bedrocktc::sdk::Vec3{mn.x, mx.y, mn.z}});

        lines.push_back({::bedrocktc::sdk::Vec3{mn.x, mn.y, mn.z}, ::bedrocktc::sdk::Vec3{mn.x, mx.y, mn.z}});
        lines.push_back({::bedrocktc::sdk::Vec3{mx.x, mn.y, mn.z}, ::bedrocktc::sdk::Vec3{mx.x, mx.y, mn.z}});
        lines.push_back({::bedrocktc::sdk::Vec3{mx.x, mn.y, mx.z}, ::bedrocktc::sdk::Vec3{mx.x, mx.y, mx.z}});
        lines.push_back({::bedrocktc::sdk::Vec3{mn.x, mn.y, mx.z}, ::bedrocktc::sdk::Vec3{mn.x, mx.y, mx.z}});

        drawLines(lines, color);
    };

    void* selectedEntity = nullptr;
    uintptr_t levelPtr = *(uintptr_t*)((uintptr_t)g_localPlayerPtr + ::bedrocktc::sdk::offsets::Actor::mLevel);
    if (levelPtr && s_hitResultGetEntity) {
        uintptr_t hitResultWrapper = levelPtr + ::bedrocktc::sdk::offsets::Level::mHitResultWrapper;
        void* hitResult = (void*)(hitResultWrapper + ::bedrocktc::sdk::offsets::HitResultWrapper::mHitResult);

        int hitType = *(int*)((uintptr_t)hitResult + ::bedrocktc::sdk::offsets::HitResult::mType);
        if (hitType == 1) {
            selectedEntity = s_hitResultGetEntity(hitResult);
        }
    }

    ::bedrocktc::sdk::Vec3 localPos = g_playerPos;
    ...
    auto renderActor = [&](void* ent) {
        ...
        if (g_hitboxMod->showEyeLine) {
            ...
            std::vector<std::pair<::bedrocktc::sdk::Vec3, ::bedrocktc::sdk::Vec3>> eyeLines;
            eyeLines.push_back({::bedrocktc::sdk::Vec3{minX, eyeHeight, minZ}, ::bedrocktc::sdk::Vec3{maxX, eyeHeight, minZ}});
            eyeLines.push_back({::bedrocktc::sdk::Vec3{maxX, eyeHeight, minZ}, ::bedrocktc::sdk::Vec3{maxX, eyeHeight, maxZ}});
            eyeLines.push_back({::bedrocktc::sdk::Vec3{maxX, eyeHeight, maxZ}, ::bedrocktc::sdk::Vec3{minX, eyeHeight, maxZ}});
            eyeLines.push_back({::bedrocktc::sdk::Vec3{minX, eyeHeight, maxZ}, ::bedrocktc::sdk::Vec3{minX, eyeHeight, minZ}});
            drawLines(eyeLines, g_hitboxMod->eyeLineColor);
        }

        if (g_hitboxMod->showLookLine) {
            ::bedrocktc::sdk::Vec2 rot = getActorRotation(ent);
            ...
            ::bedrocktc::sdk::Vec3 start = {centerX, eyeHeight, centerZ};
            float lineLen = g_hitboxMod->lookLineLength;
            ::bedrocktc::sdk::Vec3 end = {start.x + dirX * lineLen, start.y + dirY * lineLen, start.z + dirZ * lineLen};

            std::vector<std::pair<::bedrocktc::sdk::Vec3, ::bedrocktc::sdk::Vec3>> lookLines;
    ...
    if (s_actorFetchNearby) {
        ::bedrocktc::sdk::Vec3 extent = {30.0f, 30.0f, 30.0f};
...
void HitboxModule::onInit() {
    uintptr_t addr = ::bedrocktc::memory::resolve(::bedrocktc::memory::SignatureId::RenderLevel);
    if (addr != 0) {
        m_patchTarget = (void*)addr;
    }

    uintptr_t tb = ::bedrocktc::memory::resolve(::bedrocktc::memory::SignatureId::TessellatorBegin);
    if (tb) { m_tessBeginAddr = (void*)tb; s_tessBegin = (Tessellator_begin_t)tb; }

    uintptr_t tc = ::bedrocktc::memory::resolve(::bedrocktc::memory::SignatureId::TessellatorColor);
    if (tc) { m_tessColorAddr = (void*)tc; s_tessColor = (Tessellator_color_t)tc; }

    uintptr_t tv = ::bedrocktc::memory::resolve(::bedrocktc::memory::SignatureId::TessellatorVertex);
    if (tv) { m_tessVertexAddr = (void*)tv; s_tessVertex = (Tessellator_vertex_t)tv; }

    uintptr_t rm = ::bedrocktc::memory::resolve(::bedrocktc::memory::SignatureId::MeshHelpersRenderMeshImmediately2);
    if (rm) {
        s_renderMesh = (MeshHelpers_renderMeshImmediately_t)rm;
    } else {
        uintptr_t rm5 = ::bedrocktc::memory::resolve(::bedrocktc::memory::SignatureId::MeshHelpersRenderMeshImmediately);
        if (rm5) s_renderMesh = (MeshHelpers_renderMeshImmediately_t)rm5;
    }

    uintptr_t rmg = ::bedrocktc::memory::resolve(::bedrocktc::memory::SignatureId::RenderMaterialGroupCommon);
    if (rmg) {
        m_renderMaterialGroupAddr = (void*)rmg;
        uintptr_t groupAddr = resolveADRP(reinterpret_cast<uint32_t*>(rmg), 2, 0);
        if (groupAddr) {
            s_renderMaterialGroup = groupAddr + ::bedrocktc::sdk::offsets::MaterialGroup::mRenderMaterialGroupOffset;
        }
    }

    uintptr_t hrge = ::bedrocktc::memory::resolve(::bedrocktc::memory::SignatureId::HitResultGetEntity);
    if (hrge) s_hitResultGetEntity = (HitResult_getEntity_t)hrge;

    uintptr_t aip = ::bedrocktc::memory::resolve(::bedrocktc::memory::SignatureId::ActorIsPlayer);
    if (aip) s_actorIsPlayer = (Actor_isPlayer_t)aip;

    uintptr_t aii = ::bedrocktc::memory::resolve(::bedrocktc::memory::SignatureId::ActorIsInvisible);
    if (aii) s_actorIsInvisible = (Actor_isInvisible_t)aii;

    uintptr_t afn = ::bedrocktc::memory::resolve(::bedrocktc::memory::SignatureId::ActorFetchNearbyActorsSorted);
    if (afn) s_actorFetchNearby = (Actor_fetchNearbyActorsSorted_t)afn;

    ::bedrocktc::events::bus().subscribe<::bedrocktc::events::LocalPlayerTickEvent>([](auto& event) { s_hitboxTickCallback(event.player); });
}

void HitboxModule::applyPatch() {
    if (m_patched || !m_patchTarget) return;
    ::bedrocktc::hooks::install(m_patchTarget, (void*)_renderLevel_hook, (void**)&_renderLevel_orig);
    m_patched = true;
}
