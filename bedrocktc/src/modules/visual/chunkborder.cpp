...
static void _renderLevel_hook(void* _this, void* screenContext, void* a3) {
    ...
    auto drawBatchedLines = [&](const std::vector<std::pair<::bedrocktc::sdk::Vec3, ::bedrocktc::sdk::Vec3>>& lines, uint32_t color) {
    ...
    std::vector<std::pair<::bedrocktc::sdk::Vec3, ::bedrocktc::sdk::Vec3>> cornerLines;
    std::vector<std::pair<::bedrocktc::sdk::Vec3, ::bedrocktc::sdk::Vec3>> midLines;
    std::vector<std::pair<::bedrocktc::sdk::Vec3, ::bedrocktc::sdk::Vec3>> adjLines;
    ...
    if (hls > 0) {
        for (int i = 0; i <= chunkSize / hls; ++i) {
            ...
            targetList.push_back({::bedrocktc::sdk::Vec3(anchor.x + offset, bottom, anchor.z), ::bedrocktc::sdk::Vec3(anchor.x + offset, top, anchor.z)});
            targetList.push_back({::bedrocktc::sdk::Vec3(anchor.x + offset, bottom, anchor.z + chunkSize), ::bedrocktc::sdk::Vec3(anchor.x + offset, top, anchor.z + chunkSize)});
            targetList.push_back({::bedrocktc::sdk::Vec3(anchor.x, bottom, anchor.z + offset), ::bedrocktc::sdk::Vec3(anchor.x, top, anchor.z + offset)});
            targetList.push_back({::bedrocktc::sdk::Vec3(anchor.x + static_cast<float>(chunkSize), bottom, anchor.z + offset), ::bedrocktc::sdk::Vec3(anchor.x + static_cast<float>(chunkSize), top, anchor.z + offset)});
        }
    }

    if (vls > 0.0f) {
        for (float y = bottom; y <= top; y += vls) {
            midLines.push_back({::bedrocktc::sdk::Vec3(anchor.x, y, anchor.z), ::bedrocktc::sdk::Vec3(anchor.x + chunkSize, y, anchor.z)});
            midLines.push_back({::bedrocktc::sdk::Vec3(anchor.x, y, anchor.z), ::bedrocktc::sdk::Vec3(anchor.x, y, anchor.z + chunkSize)});
            midLines.push_back({::bedrocktc::sdk::Vec3(anchor.x + chunkSize, y, anchor.z), ::bedrocktc::sdk::Vec3(anchor.x + chunkSize, y, anchor.z + chunkSize)});
            midLines.push_back({::bedrocktc::sdk::Vec3(anchor.x, y, anchor.z + chunkSize), ::bedrocktc::sdk::Vec3(anchor.x + chunkSize, y, anchor.z + chunkSize)});
        }
    }
    ...
    for (int x = -1; x <= 2; ++x) {
        for (int z = -1; z <= 2; ++z) {
            ...
            adjLines.push_back({
                ::bedrocktc::sdk::Vec3(anchor.x + x * chunkSize, bottom, anchor.z + z * chunkSize),
                ::bedrocktc::sdk::Vec3(anchor.x + x * chunkSize, top, anchor.z + z * chunkSize)
            });
        }
    }
...
void ChunkBorderModule::onInit() {
    ...
    ::bedrocktc::events::bus().subscribe<::bedrocktc::events::LocalPlayerTickEvent>([](auto& event) { s_chunkBorderTickCallback(event.player); });
}
