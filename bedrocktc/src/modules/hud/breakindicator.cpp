...
void BreakIndicatorModule::onInit() {
    ::bedrocktc::events::bus().subscribe<::bedrocktc::events::GameModeActionEvent>([this](auto& event) {
        if (event.action == ::bedrocktc::events::GameModeAction::StartDestroyBlock) {
            m_gameMode = event.gameMode;
            m_breaking = m_gameMode != nullptr;
            m_progress = 0.0f;
            m_lastUpdate = std::chrono::steady_clock::now();
        } else if (event.action == ::bedrocktc::events::GameModeAction::StopDestroyBlock &&
    ...
    ::bedrocktc::events::bus().subscribe<::bedrocktc::events::LocalPlayerTickEvent>([this](auto&) {
        if (!m_breaking || !m_gameMode) return;
        float progress = *reinterpret_cast<float*>(
            reinterpret_cast<std::uintptr_t>(m_gameMode) +
            ::bedrocktc::sdk::offsets::GameMode::mDestroyProgress);
