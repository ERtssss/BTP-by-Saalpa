...
void ComboDisplay::onInit() {
    ::bedrocktc::events::bus().subscribe<::bedrocktc::events::LocalPlayerTickEvent>([](auto& event) { onTickHookCombo(event.player); });
...
void ComboDisplay::onFrame() {
    ...
    if (g_localPlayerNativeCombo) {
        int currentHurtTime = static_cast<::bedrocktc::sdk::Player*>(g_localPlayerNativeCombo)->hurtTime();
