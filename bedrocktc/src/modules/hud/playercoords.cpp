...
static void s_coordsCallback(::bedrocktc::sdk::Player* player) {
...
void PlayerCoordsModule::updateCoords(const ::bedrocktc::sdk::Vec3& pos) {
    m_currentPos = pos;
}

void PlayerCoordsModule::onInit() {
    ::bedrocktc::events::bus().subscribe<::bedrocktc::events::LocalPlayerTickEvent>([](auto& event) { s_coordsCallback(event.player); });
}
