...
static void s_debugTickCallback(void* _this) {
    ...
    uintptr_t rotComp = *(uintptr_t*)((uintptr_t)_this + ::bedrocktc::sdk::offsets::Actor::mActorRotationComponent);
    uintptr_t svc = *(uintptr_t*)((uintptr_t)_this + ::bedrocktc::sdk::offsets::Actor::mStateVectorComponent);

    float pitch = 0.f, yaw = 0.f;
    ::bedrocktc::sdk::Vec3 pos = {0.f, 0.f, 0.f};
    ...
    if (svc != 0) {
        pos = *(::bedrocktc::sdk::Vec3*)svc;
    }

    g_debugMod->updateData(yaw, pitch, pos);

    if (s_getBiome) {
        uintptr_t dimension = *(uintptr_t*)((uintptr_t)_this + ::bedrocktc::sdk::offsets::Actor::mDimension);
        if (dimension) {
            ...
            if (blockSource) {
                ...
                if (biome) {
                    std::string* biomeNamePtr = (std::string*)((uintptr_t)biome + ::bedrocktc::sdk::offsets::Biome::mHash + 8);
...
static void s_installLevelHooks(DebugMenuModule* mod) {
    if (!mod->m_levelInitHooked) {
        uintptr_t addr = ::bedrocktc::memory::resolve(::bedrocktc::memory::SignatureId::LevelInit);
        if (addr) {
            ::bedrocktc::hooks::install((void*)addr,
    ...
    if (!mod->m_levelDtorHooked) {
        uintptr_t addr = ::bedrocktc::memory::resolve(::bedrocktc::memory::SignatureId::LevelDtor);
        if (addr) {
            ::bedrocktc::hooks::install((void*)addr,
...
void DebugMenuModule::updateData(float yaw, float pitch, const ::bedrocktc::sdk::Vec3& pos) {
    m_yaw = yaw;
    m_pitch = pitch;
    m_pos = pos;
    ...
    if (m_level && s_getRuntimeActorList) {
        void* actorManager = *(void**)((uintptr_t)m_level + ::bedrocktc::sdk::offsets::Level::mActorManager);
        if (actorManager) {
...
void DebugMenuModule::onInit() {
    m_timeChanger = static_cast<TimeChangerModule*>(ModuleRegistry::get().find("bedrocktc.Time Changer"));
    ::bedrocktc::events::bus().subscribe<::bedrocktc::events::LocalPlayerTickEvent>([](auto& event) { s_debugTickCallback(event.player); });

    if (!s_getRuntimeActorList) {
        uintptr_t addrActorList = ::bedrocktc::memory::resolve(::bedrocktc::memory::SignatureId::ActorManagerList);
        if (addrActorList) {
    ...
    if (!this->m_cursorHooked) {
        uintptr_t addr = ::bedrocktc::memory::resolve(::bedrocktc::memory::SignatureId::HudCursor);
        if (addr != 0) {
            this->m_cursorPatchTarget = (void*)addr;
            ::bedrocktc::hooks::install(
    ...
    if (!s_getBiome) {
        uintptr_t addr = ::bedrocktc::memory::resolve(::bedrocktc::memory::SignatureId::BlockSourceGetBiome);
        if (addr != 0) {
...
void DebugMenuModule::onFrame() {
    ...
    if (!m_cacheInit) {
        const auto fontPath = ::bedrocktc::core::resourceDirectory() / "minecraft.ttf";
    ...
    if (m_showOverlay) {
        ...
        addLeft(std::string(::bedrocktc::Name) + " v" + std::string(::bedrocktc::Version), 0xFF55D2FF);
        addLeft("");
