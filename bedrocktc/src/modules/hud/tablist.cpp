...
static const void* getSkinImageFromActor(void* actor) {
    ...
    auto skinRef = *reinterpret_cast<void**>(reinterpret_cast<uintptr_t>(actor) + ::bedrocktc::sdk::offsets::Player::mSkin);
    if (!skinRef) return nullptr;

    auto threadOwner = *reinterpret_cast<void**>(reinterpret_cast<uintptr_t>(skinRef) + ::bedrocktc::sdk::offsets::SerializedSkinRef::mSkinImpl);
    if (!threadOwner) return nullptr;

    auto skinImpl = reinterpret_cast<uintptr_t>(threadOwner) + ::bedrocktc::sdk::offsets::ThreadOwner::mObject;
    auto image = reinterpret_cast<const void*>(skinImpl + ::bedrocktc::sdk::offsets::SerializedSkinImpl::mSkinImage);

    if (*reinterpret_cast<const bool*>(skinImpl + ::bedrocktc::sdk::offsets::SerializedSkinImpl::mIsPersona)) {
        auto begin = *reinterpret_cast<const uintptr_t*>(skinImpl + ::bedrocktc::sdk::offsets::SerializedSkinImpl::mSkinAnimatedImages);
        auto end = *reinterpret_cast<const uintptr_t*>(skinImpl + ::bedrocktc::sdk::offsets::SerializedSkinImpl::mSkinAnimatedImages + sizeof(uintptr_t));
        if (begin && end >= begin && end - begin <= ::bedrocktc::sdk::offsets::AnimatedImageData::Size * 64) {
            for (auto entry = begin; entry < end; entry += ::bedrocktc::sdk::offsets::AnimatedImageData::Size) {
                auto type = *reinterpret_cast<const uint32_t*>(entry + ::bedrocktc::sdk::offsets::AnimatedImageData::mType);
                if (type == 2 || type == 3) {
                    image = reinterpret_cast<const void*>(entry + ::bedrocktc::sdk::offsets::AnimatedImageData::mImage);
                    if (type == 3) break;
                }
            }
        }
...
static bool extractHeadFromActor(void* actor, HeadPixels& out) {
    ...
    auto width = *reinterpret_cast<const uint32_t*>(imageAddr + ::bedrocktc::sdk::offsets::SkinImage::mWidth);
    auto height = *reinterpret_cast<const uint32_t*>(imageAddr + ::bedrocktc::sdk::offsets::SkinImage::mHeight);
    auto pixels = *reinterpret_cast<const uint8_t* const*>(imageAddr + ::bedrocktc::sdk::offsets::Image::mBytesOffset);
...
static std::string getActorName(void* actor) {
    ...
    auto* filteredName = reinterpret_cast<const std::string*>(reinterpret_cast<uintptr_t>(actor) + ::bedrocktc::sdk::offsets::Actor::mFilteredNameTag);
    if (filteredName && !filteredName->empty() && filteredName->size() <= 256) {
        auto name = cleanPlayerName(*filteredName);
        if (!name.empty()) return name;
    }

    auto* playerName = reinterpret_cast<const std::string*>(reinterpret_cast<uintptr_t>(actor) + ::bedrocktc::sdk::offsets::Player::mName);
...
void TablistModule::onLocalPlayerTick(void* localPlayer) {
    ...
    auto level = *reinterpret_cast<void**>(reinterpret_cast<uintptr_t>(localPlayer) + ::bedrocktc::sdk::offsets::Actor::mLevel);
    if (level && s_getRuntimeActorList) {
        auto actorManager = *reinterpret_cast<void**>(reinterpret_cast<uintptr_t>(level) + ::bedrocktc::sdk::offsets::Level::mActorManager);
...
void TablistModule::onInit() {
    s_getRuntimeActorList = reinterpret_cast<GetRuntimeActorList_t>(::bedrocktc::memory::resolve(::bedrocktc::memory::SignatureId::ActorManagerList));
    s_actorIsPlayer = reinterpret_cast<ActorIsPlayer_t>(::bedrocktc::memory::resolve(::bedrocktc::memory::SignatureId::ActorIsPlayer));
    s_actorGetNameTag = reinterpret_cast<ActorGetNameTag_t>(::bedrocktc::memory::resolve(::bedrocktc::memory::SignatureId::ActorGetNameTag));
    ::bedrocktc::events::bus().subscribe<::bedrocktc::events::LocalPlayerTickEvent>([](auto& event) { tablistTickCallback(event.player); });
}
