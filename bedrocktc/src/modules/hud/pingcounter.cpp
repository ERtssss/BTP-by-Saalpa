...
static void _update_hook(void* _this) {
    ...
    if (g_pingMod && g_pingMod->enabled) {
        int avgPing = *(int*)((uintptr_t)_this + ::bedrocktc::sdk::offsets::RakNetConnector::mAvgPing);
        if (avgPing >= 0) {
...
void PingCounterModule::onInit() {
    ...
    uintptr_t addr = ::bedrocktc::memory::resolve(::bedrocktc::memory::SignatureId::RaknetUpdate);
    if (addr != 0) {
...
void PingCounterModule::applyPatch() {
    ...
    ::bedrocktc::hooks::install(m_patchTarget, (void*)_update_hook, (void**)&_update_orig);
    m_patched = true;
}
