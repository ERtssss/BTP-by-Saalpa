...
static void s_speedCallback(::bedrocktc::sdk::Player* player) {
...
void SpeedDisplayModule::updatePosition(const ::bedrocktc::sdk::Vec3& pos) {
    if (m_firstTick) {
        m_lastPos = pos;
        m_firstTick = false;
        m_currentSpeed = 0.0f;
...
void SpeedDisplayModule::onInit() {
    ::bedrocktc::events::bus().subscribe<::bedrocktc::events::LocalPlayerTickEvent>([](auto& event) { s_speedCallback(event.player); });
}
