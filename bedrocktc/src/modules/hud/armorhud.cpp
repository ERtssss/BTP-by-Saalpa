...
constexpr std::size_t FillingContainerItemsOffset = ::bedrocktc::sdk::offsets::Inventory::FillingContainerItems;
constexpr std::size_t ItemStackSize = ::bedrocktc::sdk::offsets::Inventory::ItemStackSize;
...
::bedrocktc::hooks::Handle hudRendererHook = nullptr;

constexpr std::array<const char*, SlotCount> HudElementIds{
    "bedrocktc.armorhud.helmet",
    "bedrocktc.armorhud.chestplate",
    "bedrocktc.armorhud.leggings",
    "bedrocktc.armorhud.boots",
    "bedrocktc.armorhud.offhand",
    "bedrocktc.armorhud.mainhand"
};
...
void* getCarriedItem(void* player) {
    ...
    if (!vtable || !vtable[::bedrocktc::sdk::offsets::VTable::PlayerGetCarriedItem]) return nullptr;
    return reinterpret_cast<void* (*)(void*)>(vtable[::bedrocktc::sdk::offsets::VTable::PlayerGetCarriedItem])(player);
}
...
void* getStackItem(void* stack) {
    ...
    return *reinterpret_cast<void**>(reinterpret_cast<std::byte*>(counter) + ::bedrocktc::sdk::offsets::ShulkerPreview::SharedCounterPointer);
}
...
short getMaxDamage(void* item) {
    ...
    return reinterpret_cast<short (*)(void*)>(vtable[::bedrocktc::sdk::offsets::VTable::ItemGetMaxDamage])(item);
}
...
void flushImages(void* context) {
    ...
    reinterpret_cast<Fn>(vtable[::bedrocktc::sdk::offsets::VTable::MinecraftUIRenderContextFlushImages])(context, color, 1.0f, material);
}
...
ArmorHudModule::~ArmorHudModule() {
    if (hudRendererHook) {
        ::bedrocktc::hooks::remove(hudRendererHook);
...
void ArmorHudModule::onInit() {
    baseActorRenderContextCtor = reinterpret_cast<BaseActorRenderContextCtorFn>(
        ::bedrocktc::memory::resolve(::bedrocktc::memory::SignatureId::BaseActorRenderContextCtor));
    itemStackBaseGetDamageValue = reinterpret_cast<ItemStackBaseGetDamageValueFn>(
        ::bedrocktc::memory::resolve(::bedrocktc::memory::SignatureId::ItemStackBaseGetDamageValue));
    itemStackBaseGetRawNameId = reinterpret_cast<ItemStackBaseGetRawNameIdFn>(
        ::bedrocktc::memory::resolve(::bedrocktc::memory::SignatureId::ItemStackBaseGetRawNameId));
    itemRendererRenderGuiItemNew = reinterpret_cast<ItemRendererRenderGuiItemNewFn>(
        ::bedrocktc::memory::resolve(::bedrocktc::memory::SignatureId::ItemRendererRenderGuiItemNew));

    const std::uintptr_t hudRenderer = pl::memory::resolveVtableFunction(
        "17HudCameraRenderer",
        ::bedrocktc::sdk::offsets::VTable::HudCameraRendererRender,
        MinecraftLibrary);
    if (hudRenderer && !hudRendererHook) {
        hudRendererHook = ::bedrocktc::hooks::install(
...
void ArmorHudModule::submitEditorElements(const ConfigSnapshot&) {
}
...
void ArmorHudModule::renderNative(void* context, void* client) {
    ...
    const struct { float width = 1200.0f; float height = 720.0f; } surface;
    ...
    if (canRender) {
        ...
        if (screenContext && game) {
            ...
            itemRenderer = *reinterpret_cast<void**>(baseActorRenderContext + ::bedrocktc::sdk::offsets::ShulkerPreview::BaseActorRenderContextItemRenderer);
        }
    }
