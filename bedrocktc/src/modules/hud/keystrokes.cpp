...
static void s_normalTickCallback(void* player) {
    ...
    auto* moveInput = ::bedrocktc::sdk::moveInputComponent(player);
    if (!moveInput) return;
...
void KeystrokesModule::onInit() {
    ::bedrocktc::events::bus().subscribe<::bedrocktc::events::LocalPlayerTickEvent>([this](auto& event) {
        resolveNativeInputTick();
        s_normalTickCallback(event.player);
    });
    ::bedrocktc::events::bus().subscribe<::bedrocktc::events::GameModeActionEvent>([this](auto& event) {
        switch (event.action) {
            case ::bedrocktc::events::GameModeAction::StartDestroyBlock:
                if (!m_destroyActive.exchange(true, std::memory_order_relaxed)) queueNativeExplicitLeft();
                break;
            case ::bedrocktc::events::GameModeAction::StopDestroyBlock:
                m_destroyActive.store(false, std::memory_order_relaxed);
                break;
            case ::bedrocktc::events::GameModeAction::Attack:
                queueNativeExplicitLeft();
                break;
            case ::bedrocktc::events::GameModeAction::Interact:
                queueNativeRight(interactSwingMask);
                break;
            case ::bedrocktc::events::GameModeAction::UseItemOn:
                break;
            case ::bedrocktc::events::GameModeAction::UseItem:
                if (event.hasNativeResult && event.nativeResult) queueNativeRight(useItemSwingMask);
                break;
            case ::bedrocktc::events::GameModeAction::StartBuildBlock:
            case ::bedrocktc::events::GameModeAction::UseItemAsAttack:
                break;
        }
    });

    if (!m_playerSwingHooked) {
        const auto address = ::bedrocktc::memory::resolve(::bedrocktc::memory::SignatureId::LocalPlayerSwing);
        if (address) {
            const auto handle = ::bedrocktc::hooks::install(reinterpret_cast<void*>(address), reinterpret_cast<void*>(playerSwingDetour), reinterpret_cast<void**>(&s_playerSwingOriginal));
