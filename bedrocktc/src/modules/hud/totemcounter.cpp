...
void TotemCounterModule::onInit() {
    ::bedrocktc::core::InventoryAccess::get().initialize();
    ::bedrocktc::events::bus().subscribe<::bedrocktc::events::LocalPlayerTickEvent>(
        [this](auto& event) { updateCount(event.player); }
    );
}

void TotemCounterModule::updateCount(::bedrocktc::sdk::Player* player) {
    if (!enabled) return;
    m_count.store(::bedrocktc::core::InventoryAccess::get().countItems(player, ItemIdentifier),
...
void TotemCounterModule::onMenuRegistered() {
    ...
    builder.modId("btp").description(description).defaultEnabled(masterEnabled)
        ...
        .onToggle([this](std::string_view, bool value){ setMasterEnabled(value); });
